  ...
  <div id="borrar">
  	<div style="float:left;">
    	...
    	<input type="checkbox" id="check_foto_<?=$this->nombre?>" name="borrar_<?=$this->nombre?>" value="1"<?
			if($valor=="")
				echo "disabled";
			   ?> onclick="if(this.checked){document.getElementById('cuadro_<?=$this->nombre?>').prototype=document.getElementById('cuadro_<?=$this->nombre?>').src;document.getElementById('cuadro_<?=$this->nombre?>').src='images/imagen_default.jpg';}else{document.getElementById('cuadro_<?=$this->nombre?>').src=document.getElementById('cuadro_<?=$this->nombre?>').prototype;}" />
	</div>
