<?php
	...
	while($fila_combo=mysql_fetch_array($result_combo)){
		echo "<option value=\"".$fila_combo[strtoupper($id_auto)]."\" ";
		if(isset(${$id_combo})){
			if(${$id_combo}==$fila_combo[strtoupper($id_auto)])
				echo "selected";
			if($accion!="m" && $id_defecto == $fila_combo[strtoupper($id_auto)])
				echo "selected";
		}
