(function($) {
	...
	*/
	$.fn.agileUploaderAttachFile = function(file) {
		...
		$('#agileUploaderFileList').append('<li id="id-'+file.uid+'" class="'+alt+'"><div id="'+file.uid+'CurrentProgress" class="agileUploaderCurrentProgress"></div><div class="agileUploaderFilePreview" style="display: none;"></div><div class="agileUploaderFileName" style="display: none;">'+file.fileName+'</div><div class="agileUploaderFileSize" style="display: none;"></div><div class="agileUploaderRemoveFile" style="display:none;"><a href="#" id="remove-'+file.uid+'" onClick="document.getElementById(\'agileUploaderSWF\').removeFile(\''+file.uid+'\'); return false;"><img class="agileUploaderRemoveIcon" src="'+opts.removeIcon+'" alt="remove" /></a></div></li>');
	...
	*/
	$.fn.agileUploaderCurrentEncodeProgress = function(file) {
		...
		if(file.percentEncoded >= 100) {
			...
			// add remove all
			$('#agileUploaderRemoveAll').html('<a href="#" onClick="document.getElementById(\'agileUploaderSWF\').removeAllFiles(); $(\'#agileUploaderFileList\').empty(); $(\'#agileUploaderRemoveAll\').empty(); return false;"><img src="js_library/jq_multipleuploads/remove-all.png" alt="" /></a>');
		}
	}
	...
	*/
	$.fn.agileUploader = function(options) {			
		...
		return this.each(function() {
			...
			$('#'+this.id).prepend('<div id="agileUploaderInfo"><ul id="agileUploaderFileList"></ul></div><div id="agileUploaderRemoveAll"></div>');
		});	
	}
	...
	*/
	...
	$.fn.agileUploader.defaults = {
		...
		flashSrc: 'js_library/jq_multipleuploads/agile-uploader.swf',
		flashWidth: 156,
		flashHeight: 40,
		...
		flashVars: {
			...
			file_limit: -1,
			button_up:'js_library/jq_multipleuploads/add-file.png',
			button_over:'js_library/jq_multipleuploads/add-file.png',
			button_down:'js_library/jq_multipleuploads/add-file.png'		
		},
		progressBarColor: '#000000',
		attachScrollSpeed: 1000,		
		removeIcon: 'images/ico-delete-trans.png',
		genericFileIcon: 'js_library/imagenes/file-icon.png',
