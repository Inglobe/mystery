<?
 ...
 class LTChatDataKeeper
 {
   ...
   function user_logged_in()
   {
       if($_SESSION['LTChart_user_id'] != null && $_SESSION['LTChart_user_nick'] != null)
         return true;
  	   	 ...
  	   	 if($_SESSION['id_usr'] != null && $_SESSION['usr_user'] != null){
  	   	 	$_SESSION['LTChart_user_id'] = $_SESSION['id_usr'];
  	   	 	$_SESSION['LTChart_user_nick'] = $_SESSION['usr_user'];
  	   	 	return true;
		 }
		 else{
		 	return false;
   ...
   function get_statistics()
   {
   	 ...
   	 $result = mysql_query("select count(id_usuario) as users from usuarios where kicked != '1' and chat_id = '".LTChat_Main_CHAT_ID."'") or debug(mysql_error(), "LTChatDataKeeper", __LINE__);
   	 echo mysql_error();
   	 if($row = mysql_fetch_object($result))
   	   $stats['registered_users'] = $row->users;

   	 $result = mysql_query("select * from usuarios where kicked != '1' and chat_id = '".LTChat_Main_CHAT_ID."' order by id_usuario desc limit 0,1") or debug(mysql_error(), "LTChatDataKeeper", __LINE__);
   	 if($row = mysql_fetch_object($result))
   ...
   function delete_user($id)
   {
	  	...
	  	mysql_query("delete from usuarios where id_usuario = '{$id}' and chat_id = '".LTChat_Main_CHAT_ID."'") or debug(mysql_error(), "LTChatDataKeeper", __LINE__);
   }
   ...
   function login_user($login, $password, $guest)
   {
	   ...
	   $result = mysql_query("select * from usuarios where user = '{$login}' and chat_id = '".LTChat_Main_CHAT_ID."'") or debug(mysql_error(), "LTChatDataKeeper", __LINE__);
	   $time = time();

	   if($row = mysql_fetch_object($result))
	   {
	   	  if($password != $row->pass && md5($password) != $row->pass)
	   	    return array('error' => ChDK_log_err_bad_password);

	   	  if($row->reason != "")
	   	    return array('error' => ChDK_log_err_banned, 'reason' => $row->reason);

		  $_SESSION['LTChart_user_id'] = $row->id_usuario;
		  $_SESSION['LTChart_user_nick'] = $row->user;
		  $_SESSION['LTChart_user_rights'] = $row->rights;
   	      mysql_query("update usuarios set last_seen = '".time()."', last_host = '{$hostname}', last_ip = '{$_SERVER['REMOTE_ADDR']}' where id_usuario = '{$_SESSION['LTChart_user_id']}' and chat_id = '".LTChat_Main_CHAT_ID."'") or debug(mysql_error(), "LTChatDataKeeper", __LINE__);

   	      // usuniecie niepotrzebnych smieci
		  /*$result = mysql_query("select * from usuarios where (( rights = 'Standard' and last_seen < ".time()." - ".get_ConfVar("ChDK_delete_user_after").") or
		  ...
		  mysql_query("update usuarios set foto = '' where rights = 'Standard' and last_seen < ".time()." - ".ChDK_delete_free_avatar_after." and chat_id = '".LTChat_Main_CHAT_ID."'") or debug(mysql_error(), "LTChatDataKeeper", __LINE__);*/
		  return true;
	   }
   ...
   function add_user($login, $password, $other_options = array(), $guest = 0)
   {
	   ...
	   mysql_query("insert into usuarios(user,password,last_seen, rights,chat_id) values ('{$login}', '{$password}','".time()."','{$rights}','".LTChat_Main_CHAT_ID."')") or debug(mysql_error(), "LTChatDataKeeper", __LINE__);

	   if(mysql_affected_rows() == -1)
	     return false;
	   else
	   {
	     $result = mysql_query("select * from usuarios where user = '{$login}' and password = '{$password}' and chat_id = '".LTChat_Main_CHAT_ID."'") or debug(mysql_error(), "LTChatDataKeeper", __LINE__);
   ...
   function user_action($room)
   {
  	 ...
  	 {
   	   mysql_query("update usuarios set last_seen = '".time()."' where id_usuario = '{$_SESSION['LTChart_user_id']}' and chat_id = '".LTChat_Main_CHAT_ID."'") or debug(mysql_error(), "LTChatDataKeeper", __LINE__);
   ...
   {
   	 $result = mysql_query("SELECT U.user as user, U.id_usuario as user_id, PT.id, PT.users_id_from, PT.users_id_to, PT.text, PT.time, PT.delivered_from, PT.delivered_to FROM `usuarios` U , `".LTChat_Main_prefix."private_talk` PT WHERE PT.users_id_from = U.id_usuario and PT.users_id_to = '{$_SESSION['LTChart_user_id']}' and PT.id > '{$last_id}'  and delivered_from = '1' and delivered_to = '0' and time + 10 < ".time()." and PT.chat_id = '".LTChat_Main_CHAT_ID."' and U.chat_id = '".LTChat_Main_CHAT_ID."' order by id_usuario desc") or debug(mysql_error(), "LTChatDataKeeper", __LINE__);
   ...
   function set_avatar($file_path)
   {
     ...
     mysql_query("update usuarios set foto = '{$file_path}' where id_usuario = '{$_SESSION['LTChart_user_id']}' and chat_id = '".LTChat_Main_CHAT_ID."'") or debug(mysql_error(), "LTChatDataKeeper", __LINE__);
   ...
   {
     $result = mysql_query("select user, foto from usuarios where foto <> '' and chat_id = '".LTChat_Main_CHAT_ID."'") or debug(mysql_error(), "LTChatDataKeeper", __LINE__);
     while ($row = mysql_fetch_array($result,MYSQL_ASSOC))
       $out[$row['foto']] = $row['user'];
   ...
   function get_user_by_nick($nick, $simple = false)
   {
     ...
     $result = mysql_query("select * from usuarios where user = '{$nick}' and chat_id = '".LTChat_Main_CHAT_ID."'") or debug(mysql_error(), "LTChatDataKeeper", __LINE__);
   ...
   {
   	 $id = addslashes($id);
     $result = mysql_query("select * from usuarios where id_usuario = '{$id}' and chat_id = '".LTChat_Main_CHAT_ID."'") or debug(mysql_error(), "LTChatDataKeeper", __LINE__);
...
//------------ rooms ---------------------------------------------------
   ...
   function kick_user($user_id, $reason)
   {
     ...
     mysql_query("update usuarios set kicked = '1', reason = '{$reason}' where id_usuario='{$user_id}' and chat_id = '".LTChat_Main_CHAT_ID."'") or debug(mysql_error(), "LTChatDataKeeper", __LINE__);
   }
   ...
   function get_all_rooms()
   {
     ...
     $result = mysql_query("select usuarios.id_usuario, user, room from `".LTChat_Main_prefix."who_is_online`,`usuarios` where users_id = usuarios.id_usuario and online = '1' and `".LTChat_Main_prefix."who_is_online`.`chat_id` = '".LTChat_Main_CHAT_ID."' and `usuarios`.`chat_id` = '".LTChat_Main_CHAT_ID."'") or debug(mysql_error(), "LTChatDataKeeper", __LINE__);
     echo mysql_error();
     while ($row = mysql_fetch_object($result))
     {
     	for($i = 0; $i < count($out); $i++)
     	{
     	  ...
     	  {
     	    $out[$i]->users_online[] = array('nick' => $row->user, 'id' => $row->id);
     	    break;
     	  }
     	}
     	if($out[$i]->room_name != $row->room)
     	  $undefined[$row->room][] = $row->user;
     }
...
//----------------- friend ---------------------------------------------
   ...
   {
   	 $result = mysql_query("select U.id_usuario as users_id, U.user, U.`foto` from usuarios as U, ".LTChat_Main_prefix."friends as F where F.from_users_id = '{$_SESSION['LTChart_user_id']}' and F.to_users_id = U.id_usuario and F.chat_id = '".LTChat_Main_CHAT_ID."' and U.chat_id = '".LTChat_Main_CHAT_ID."'") or debug(mysql_error(), "LTChatDataKeeper", __LINE__);
	 while($row = mysql_fetch_object($result))
	   $out['from'][] = $row;

	 $result = mysql_query("select U.user, F.from_users_id from usuarios as U, ".LTChat_Main_prefix."friends as F where F.to_users_id = '{$_SESSION['LTChart_user_id']}' and F.from_users_id = U.id_usuario and F.chat_id = '".LTChat_Main_CHAT_ID."' and U.chat_id = '".LTChat_Main_CHAT_ID."'") or debug(mysql_error(), "LTChatDataKeeper", __LINE__);
	 while($row = mysql_fetch_object($result))
	   $out['to'][] = $row;
...
################## ignore #############################################
   ...
   function post_shoutbox_msg($text, $login, $shoutbox_id)
   {
   	   ...
   	   $login = addslashes($login);
       mysql_query("INSERT INTO `".LTChat_Main_prefix."shoutbox` (text, user, shout_id, chat_id) values ('{$text}', '{$login}', '{$shoutbox_id}','".LTChat_Main_CHAT_ID."')") or debug(mysql_error(), "LTChatDataKeeper", __LINE__);
   }

   function post_private_msg($text, $private_id, $delivered)
   {
   	 mysql_query("update usuarios set posted_msg = posted_msg + 1 where id_usuario = '{$_SESSION['LTChart_user_id']}'and chat_id = '".LTChat_Main_CHAT_ID."'") or debug(mysql_error(), "LTChatDataKeeper", __LINE__);
   ...
   function get_users_list_from_array($users)
   {
	  ...
	  $time = time();
        ...
        $query_select = "SELECT `O`.`who_id` as id, `O`.`users_id`, `O`.`online`, `O`.`room`, `O`.`action_time`, U.`user`,U.`foto`,U.`id_usuario` as user_id FROM `".LTChat_Main_prefix."who_is_online` O, `usuarios` U WHERE O.users_id = U.id_usuario and (1=2 {$where}) and `O`.`chat_id` = '".LTChat_Main_CHAT_ID."' and O.online = '1' and U.chat_id = '".LTChat_Main_CHAT_ID."' order by who_id asc";
   ...
   function get_users_list($room = NULL, $who_id = 0)
   {
	 ...
	 if($who_id == 0)	   $query_select = "SELECT `O`.`who_id` as id, `O`.`users_id`, `O`.`online`, `O`.`room`, `O`.`action_time`, U.`user`,U.`foto`,U.`id_usuario` as user_id FROM `".LTChat_Main_prefix."who_is_online` O, `usuarios` U WHERE O.users_id = U.id_usuario and O.online = '1' and `O`.`room` = '{$room}' and `O`.`chat_id` = '".LTChat_Main_CHAT_ID."' and U.chat_id = '".LTChat_Main_CHAT_ID."' order by who_id asc";
	 else				   $query_select = "SELECT `O`.`who_id` as id, `O`.`users_id`, `O`.`online`, `O`.`room`, `O`.`action_time`, U.`user`,U.`foto`,U.`id_usuario` as user_id FROM `".LTChat_Main_prefix."who_is_online` O, `usuarios` U WHERE O.users_id = U.id_usuario and O.who_id > '{$who_id}' and `O`.`room` = '{$room}' and `U`.`chat_id` = '".LTChat_Main_CHAT_ID."' and `O`.`chat_id` = '".LTChat_Main_CHAT_ID."' group by online order by who_id asc";
   ...
   function get_shoutbox_elements($sbox_id,  $lastid)
   {
	 ...
	 $result = mysql_query("select *,".LTChat_Main_prefix."shoutbox.user as user from ".LTChat_Main_prefix."shoutbox where shout_id = '{$sbox_id}' and id > '{$lastid}' and chat_id = '".LTChat_Main_CHAT_ID."' order by id desc limit 0, {$limit}") or debug(mysql_error(), "LTChatDataKeeper", __LINE__);
	 while ($row = mysql_fetch_object($result))
	   $out[] = $row;
   ...
   function get_prv_msg_elements($lastid, $private_id = -1)
   {
   	   ...
   	   $result = mysql_query("SELECT U.user as user, PT.id, PT.users_id_from, PT.users_id_to, PT.text, PT.time, PT.delivered_from, PT.delivered_to FROM `usuarios` U , `".LTChat_Main_prefix."private_talk` PT WHERE  PT.chat_id = '".LTChat_Main_CHAT_ID."' and U.chat_id = '".LTChat_Main_CHAT_ID."' and PT.users_id_from = U.id_usuario  and PT.id > '{$lastid}' and ( ( PT.users_id_from = '{$_SESSION['LTChart_user_id']}' and delivered_from = '0' ) or ( PT.users_id_to = '{$_SESSION['LTChart_user_id']}' and delivered_to = '0' ) ) and (PT.users_id_from = '{$private_id}' or PT.users_id_to = '{$private_id}') order by id desc") or debug(mysql_error(), "LTChatDataKeeper", __LINE__);
