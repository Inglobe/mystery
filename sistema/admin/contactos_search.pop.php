...
<div id="titulo">
  ...
  ?> </span><span class="separador_tit">/</span><span class="gris"> Contactos </span><span class="separador_tit">/</span><span class="gris_claro">Search</span> </h1>
</div>
<div id="btns_sup"><a href="contactos_am.pop.php?accion=a&amp;id=<?=$_GET["id"]?>"><img src="imagenes/btn_sup_add.jpg" alt="" border="0" /></a></div>
<div id="feed">
        ...
        <div id="tabla">
          <table width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            <?
				...
				$consulta = "SELECT * FROM contactos WHERE id_cliente = ".$_GET["id"];
				$result = mysql_query($consulta." LIMIT ".$ls.",".$paginacion,$link);
				echo mysql_error($link);
				while($fila = mysql_fetch_assoc($result)){
            		$estilo = ($estilo=="lista_clara"?"lista_oscura":"lista_clara");
					$link_modificar = "contactos_am.pop.php?id_modificar=".$fila["id_contacto"]."&amp;accion=m&amp;id=".$_GET["id"];
	            	$link_borrar = "contactos_procesar.pop.php?id_eliminar=".$fila["id_contacto"]."&amp;accion=d&amp;id=".$_GET["id"];
            ?>
              <tr class="<?=$estilo?>">
			  	<td><a href="<?=$link_modificar?>"><?=$fila["nombre"]?></a></td>
                <td><a href="<?=$link_modificar?>"><?=$fila["email"]?></a></td>
