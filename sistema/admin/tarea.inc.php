<?
	if($_SESSION["usr_tipo"]==2 && $fila_proyecto["id_estado_proyecto"] == 3){
		$href_tarea="javascript:alert('No se puede editar un proyecto finalizado.')";
	}
	else{
...
<div class="descripcion<?=($tiene_hijos?"_titulo":"")?>"><a href="<?=$href_tarea?>" class="<?=$estilo_tarea_permiso?>" title="Shopper: <?=$fila["usuario_responsable"]?> - Autor: <?=$fila["usuario_owner"]?>"><?=$fila["descripcion"]?></a></div>
<?
...
<div class="estado"><?=$fila["tiempo"]?></div>
<?
if($fila["obs_fin"]!=""){
?>
