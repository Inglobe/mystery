...
<title><h1><span class="gris">Auditorias</span><span class="separador_tit">/</span><span class="gris"><?=$fila_proyecto["nombre"]?></span><span class="separador_tit">/</span><span class="gris_claro">Tarea</span></h1></title>
</head>
<body>
<?
...
<form id="form_agregar_tarea" name="form_agregar_tarea" method="post" action="tarea_feed.pop.php?id_proyecto=<?=$_GET["id_proyecto"]?><?=(isset($_GET["id_tarea"])?"&amp;id_tarea=".$_GET["id_tarea"]:"")?>" enctype="multipart/form-data">
  ...
  ?></div>
          ...
          <div class="campo" <?=($_SESSION["usr_tipo"]!=1?'style="display:none"':'')?>>
            <label for="cmb_id_usuario_responsable">Shopper:</label>
		  ...
		  ?>
		  <div class="campo" style="width:120px; display: none;">
		  ...
		  ?>
		  <div class="campo" style="width:110px; display: none;">
		  ...
		  ?>
		  <div class="campo" style="margin-bottom:10px; width:160px;">
        ...
        ?>
		  ...
		  ?>
		  <div class="campo" style="margin-bottom:0px;<?=($_SESSION["usr_tipo"]==2?'display:none;':'')?>" >
		  	<label for="ocultar">Ocultar cliente</label><input type="checkbox" id="ocultar" name="ocultar" value="1" <?=($fila_tarea["ocultar"]==1?'checked="checked"':'')?> />
		  </div>
        </div>
      </div>
    </div>
    ...
    <div class="borde_bloque_izq">
      <div class="borde_bloque_der">
        ...
        <div class="control">
	            ...
	            <input type="text" name="descripcion" id="descripcion" value="<?=xhtmlOut($fila_tarea["descripcion"])?>" style="width: 590px;" <?=($_SESSION["usr_tipo"]!=1?'readonly="readonly"':'')?> />
              </div>
        	<div style="width: 370px; float: left;">
	          <div class="control" style="padding-bottom:0px;">
	            ...
	            <textarea name="obs" id="obs" style="width: 350px; height:45px;"><?=$fila_tarea["obs"]?></textarea>
	          </div>
	        </div>
	        <div style="width: 225px; float: left;">
	          <div class="control" style="padding-bottom:0px;">
	            <div class="label_add" style="float: none; text-align: left">
	              <label for="detalle" style="color:#CC3333">Detalle de la tarea: </label>
	            </div>
	            <textarea name="detalle" id="detalle" style="width: 220px; height:45px; <?=($_SESSION["usr_tipo"]==2?'display:none':'')?>"><?=$fila_tarea["detalle"]?></textarea>
				<?
				if($_SESSION["usr_tipo"]==2){
				?>
				<div><?=$fila_tarea["detalle"]?></div>
				<?
				}
				?>
	          </div>
            ...
            <div class="control">
	            <div style="padding-top: 10px; float:left;"><div class="label_add" style="float: none; text-align: left">
	              <label for="obs">Archivos: </label>
	            </div>
	            <input type="file" name="multifile" id="multifile" />
                </div>
	            <div style="padding-top: 10px; float:left; width:390px;">
	              <ul id="file_list"><?
  ...
  <div class="bloque_sin_degrade"  style="padding-bottom: 5px;">
    ...
    <div class="borde_bloque_izq">
      <div class="borde_bloque_der">
		...
		?>
        <div class="campos" style="float:left; width:120px;">
        <?
			$consulta_et = "SELECT * FROM estados_tarea ".($_SESSION["usr_tipo"]!=1?" WHERE id_estado_tarea < 3":"")." ORDER BY id_estado_tarea ASC";
		...
		<?
		if(!$tiene_hijos){
			if($_SESSION["usr_tipo"]==1){//admin
		?>
        <div class="campos" style="float:left; width:220px;" >
		  <div class="campo" style="margin-bottom:0px;">
          	<div style="padding-bottom:10px;">
                <label for="tiempo" style="float:left; margin-top:2px; margin-right:4px; display:block; width:100px;">Puntaje:</label>
                <input type="text" name="tiempo" id="tiempo" value="<?=$fila_tarea["tiempo"]?>" style="width: 20px;" /> 
			</div>
            <div>
                <label for="tiempo_estimado" style="float:left; margin-top:2px; margin-right:4px; display:block; width:100px;">Puntajes permitidos:</label>
                <input type="text" name="tiempo_estimado" id="tiempo_estimado" value="<?=$fila_tarea["tiempo_estimado"]?>" style="width: 90px;" />
            </div>
          </div>
        </div>
        <div class="campos" style="float:left; width:200px;" >
		  <div class="campo" style="margin-bottom:0px;">
          	<div>
                <label for="tiempo" style="float:left; margin-top:2px; margin-right:4px; display:block;">Leyenda:</label>
                <textarea name="leyenda" id="leyenda" cols="" rows="" style="width:180px;"><?=$fila_tarea["leyenda"]?></textarea>
			</div>
          </div>
        </div>
		<?
		...
		?>
        <div class="campos" style="float:left; width:480px;" >
		  <div class="campo" style="margin-bottom:0px; float:left; width:90px;">
            <label for="tiempo" style="float:left; margin-top:2px; margin-right:4px; display:block;">Puntaje:</label>
			<?
			$puntajes = explode(",",$fila_tarea["tiempo_estimado"]);
			?>
            <select name="tiempo" id="tiempo">
			<?
			foreach($puntajes as $puntaje){
			?>
			  <option value="<?=$puntaje?>" <?=($puntaje==$fila_tarea["tiempo"]?'selected="selected"':'')?>><?=$puntaje?></option>
			<?
			}
			?>
			</select>
			<input type="hidden" name="tiempo_estimado" id="tiempo_estimado" value="<?=$fila_tarea["tiempo_estimado"]?>" />
			<textarea name="leyenda" id="leyenda" cols="" rows="" style="width:180px; display: none;"><?=$fila_tarea["leyenda"]?></textarea>
          </div>
          <div style="float:left; width:380px; overflow:auto; height:46px"><?=xhtmlOut($fila_tarea["leyenda"])?></div>
        </div>
        <?
			}
		}
