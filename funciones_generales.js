...
function MYSQL2XML(query, mostrar_output){
	var oXML;
	...
	var cadena_query = "includes/xml_query.php?query=" + encodeURI(query) + "&rand_" + aleatorio(0,99999);
	oXML.open("get",cadena_query ,false);
	oXML.send(null);
	...
	if(resultado.getElementsByTagName('error').length != 0){
		alert(resultado.getElementsByTagName('error')[0].firstChild.data + "\nQuery: " + query);
	}
	else{
		if(mostrar_output){
