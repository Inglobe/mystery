...
<div id="titulo">
  ...
  ?> </span><span class="separador_tit">/</span><span class="gris"> Pagos </span><span class="separador_tit">/</span><span class="gris_claro">Search</span> </h1>
</div>
<div id="btns_sup"><a href="pagos_am.pop.php?accion=a&amp;id=<?=$_GET["id"]?>"><img src="imagenes/btn_sup_add.jpg" alt="" border="0" /></a></div>
<div id="feed">
        ...
        <div id="tabla">
          <table width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            <?
				...
				$consulta = "SELECT *,DATE_FORMAT(fecha,'%d/%m/%Y') AS fecha_f FROM pagos WHERE id_proveedor = ".$_GET["id"];
				$result = mysql_query($consulta." LIMIT ".$ls.",".$paginacion,$link);
				echo mysql_error($link);
				while($fila = mysql_fetch_assoc($result)){
            		$estilo = ($estilo=="lista_clara"?"lista_oscura":"lista_clara");
					$link_modificar = "pagos_am.pop.php?id_modificar=".$fila["id_pago"]."&amp;accion=m&amp;id=".$_GET["id"];
	            	$link_borrar = "pagos_procesar.pop.php?id_eliminar=".$fila["id_pago"]."&amp;accion=d&amp;id=".$_GET["id"];
            ?>
              <tr class="<?=$estilo?>">
			  	<td><a href="<?=$link_modificar?>"><?=$fila["fecha_f"]?></a></td>
                <td><a href="<?=$link_modificar?>"><?=$fila["monto"]?></a></td>
				<?
					$fila_tmp = mysql_fetch_assoc(mysql_query("SELECT descripcion FROM tipos_pago WHERE id_tipo_pago = ".$fila["id_tipo_pago"],$link))
				?>
                <td><a href="<?=$link_modificar?>"><?=$fila_tmp["descripcion"]?></a></td>
