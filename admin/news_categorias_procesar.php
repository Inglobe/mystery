<?
	//PARAMETROS
	$nombre_abm="news_categorias";
	$tabla_nombre="news_categorias";
	...
	function actualizar_secciones_categorias($id,$mat_ids){
		global $link;

		mysql_query("DELETE FROM secciones_categorias WHERE id_categoria = ".$id,$link);
		echo mysql_error($link);

		if(is_array($mat_ids)){
			foreach($mat_ids as $id_rel){
				$consulta = "INSERT INTO secciones_categorias VALUES(".$id_rel.",".$id.")";
	...
	$datos = makeDatosByPost($_POST,$_FILES);
 	...
 	switch($abm_accion){
 		case "a":
			...
			actualizar_secciones_categorias($id_nuevo,$_POST["ids_secciones"]);
		...
		case "d":
			//HAY QUE BORRAR TODAS LAS FOTOS DE LA GALERIA!!!!!
