<?
	...
	function actualizar_galerias_news($id,$mat_ids){
		global $link;
 	...
 	switch($_GET["abm_accion"]){
 		case "a":
			...
			actualizar_galerias_news($id_new,$_POST["ids_galerias"]);
		...
		case "m":
			...
			actualizar_galerias_news($_GET["id"],$_POST["ids_galerias"]);
		break;
		...
		case "d":
			...
			$stringq="DELETE FROM galerias_news WHERE id_new = ".$_GET["id"];
			mysql_query($stringq,$link);
			echo mysql_error($link);
