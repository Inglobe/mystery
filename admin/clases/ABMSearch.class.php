	...
	private function renderTablaResultados(){
		...
		<div id="tabla">
            ...
            ?>
				...
				if($this->campo_orden !== ""){
                ?>
                  <td class="<?=$estilo_fila?>"><a href="index.php?put=<?=$this->nombre?>_search&abm_accion=s&direccion=up&id=<?=$fila[$this->id_tabla]?>"><img src="imagenes/flecha_arriba.gif" border="0" /></a>&nbsp;<a href="index.php?put=<?=$this->nombre?>_search&abm_accion=s&direccion=down&id=<?=$fila[$this->id_tabla]?>"><img src="imagenes/flecha_abajo.gif" border="0" /></a></td>
                <?
                }
                if($this->tiene_galeria){
                ?>
